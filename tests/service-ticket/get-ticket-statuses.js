...
exports.setup = temp.setupTicket.bind('get_ticket_statuses test');

exports.get_ticket_statuses = function (test) {
	var options = {
		SrServiceRecID: temp.ticket.Id		// gets the statuses available for the ticket specified
	};

	api.action('GetTicketStatusesAction', options, function (error, result) {
		if (error) { console.error(error); }
		test.ok(!error);
		test.ok(result);
		if (result) {
		    console.dir(result.GetTicketStatusesAction.Statuses);
		}
		test.done();
	});
};
